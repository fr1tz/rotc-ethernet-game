...
//**************************************************
// Mission init script for ROTC: Infantry missions
//**************************************************

exec("./rotc-types.cs");

$Game::GameType = $Game::Infantry;
$Game::GameTypeString = "Infantry (Ethernet) 0.0+dev";
...
function executeGameScripts()
{
	...
	exec("game/server/inf/exec.cs");
	exec("game/server/blueprints/exec.cs");
	exec("game/server/weapons/weapons.cs");
	exec("game/server/weapons/stg90/exec.cs");
	exec("game/server/weapons/ff3/exec.cs");
}

function loadManual()
{
   constructManual("game/server/inf/manual/index.idx");
}

function loadHints()
{
   constructHints("game/server/inf/manual/tips.rml");
}
